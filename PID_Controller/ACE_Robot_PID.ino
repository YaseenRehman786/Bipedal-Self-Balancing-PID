...
// ========== RC Throttle Filtering Constants (Original Method) ==========
#define ALPHA  0.19  // Exponential smoothing for throttle left
#define ALPHA2 0.50  // Exponential smoothing for throttle right
...
// ========== RC Input Pins ==========
// Channel 1 (throttle right) will use pulseIn (original method)
// Channels 2 (throttle left), 3 (kill switch), & 4 (desired angle) will use interrupts
const int receiver_pin  = 24;  // Throttle right (channel 1)
const int receiver_pin2 = 26;  // Throttle left  (channel 2) -> new method with interrupts
const int receiver_pin3 = 29;  // Kill switch or mode (channel 3)
const int receiver_pin4 = 27;  // Desired angle adjustments (channel 4)

// --- For channel 1, we use pulseIn so no interrupt variables here ---
unsigned long pulse_duration;   // Channel 1 (throttle right)

// --- For channels 2, 3, and 4, we use interrupt-based reading ---
...
unsigned long pulse_duration2;  // For channel 2 (throttle left)
unsigned long pulse_duration3;  // For channel 3 (kill switch)
...
void setup() {
  ...
  // Attach interrupts for channels 2, 3, and 4
...
void loop(){
  // --- For channel 1 (right throttle), use pulseIn (original method) ---
  pulse_duration = pulseIn(receiver_pin, HIGH);
  
  // --- For channel 2 (left throttle), use the interrupt-based method ---
  unsigned long localPulseCh2;
  noInterrupts();
    localPulseCh2 = pulseWidthCh2;
  interrupts();
  pulse_duration2 = localPulseCh2;
  
  // --- For channels 3 & 4, use the interrupt-based method ---
  unsigned long localPulseCh3, localPulseCh4;
  noInterrupts();
    localPulseCh3 = pulseWidthCh3;
    localPulseCh4 = pulseWidthCh4;
  interrupts();
  pulse_duration3 = localPulseCh3; // Kill switch (channel 3)

  // Process channel 4 (desiredAngleADD) using ranges:
  ...
  // --- Original filtering for Throttle Right (Channel 1) ---
  float throttleRight = 0.2 - ((float)(pulse_duration - 999) * 0.4 / 1001.0);
  throttleRight = round(throttleRight / 0.001) * 0.001;
  filteredThrottleRight = ALPHA2 * throttleRight + (1 - ALPHA2) * filteredThrottleRight;
  velocitySetpoint = 15.0 * filteredThrottleRight;

  // --- Original filtering for Throttle Left (Channel 2) ---
  float throttleLeft = 1.0 - ((float)pulse_duration2 - 999.0) * (2.0 / 1001.0);
  ...
  Serial.print("pulse_duration (fwd/bck): ");  Serial.println(pulse_duration);
  Serial.print("pulse_duration2 (turn):  ");  Serial.println(pulse_duration2);
  Serial.print("pulse_duration3 (kill?): ");  Serial.println(pulse_duration3);
